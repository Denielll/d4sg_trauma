...
models_risk = Blueprint('models_risk', __name__ )
@models_risk.route("/api/models/risk", methods=["POST"])
def risk_call():
	try:
	...
	def main_predict_fall(gener, age, arrival_1, OHCA, weekday):
	    clf = load_pickle(path+"0415_Risk_ageCon_fallRandomForest_clf")
	    enc = load_pickle(path+"0415_Mortality_ageCon_Fall_enc")
	...
	def main_predict_car(gener, age, arrival_1, OHCA, weekday):
	    clf = load_pickle(path+"0415_risk_car_clfRandomForest_clf")
	    enc = load_pickle(path+"0415_Mortality_ageCon_car_enc")
	    inputx2 = enc.transform([[gener, arrival_1, OHCA, weekday]]).toarray()
	    inputa =  list([age]) + list(inputx2[0])
	    return clf.predict([inputa])[0]
